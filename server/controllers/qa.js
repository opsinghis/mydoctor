...
export const create = async (req, res) => {
  try {
    ...
    if (userId != req.auth._id) return res.status(400).send("Unauthorized");
...
export const deleteLessonQa = async (req, res) => {
  try {
    ...
    if (postedBy != req.auth._id) return res.status(400).send("Unauthorized");
...
export const addAnswer = async (req, res) => {
  ...
  try {
    ...
    let updated = await Qa.findByIdAndUpdate(
      questionId,
      {
        $push: { answers: { content, postedBy: req.auth._id } },
      },
      { new: true }
    )
      .populate("postedBy", "_id name")
...
export const getUserQas = async (req, res) => {
  try {
    const qas = await Qa.find({ postedBy: req.auth._id })
      .populate({
        path: "answers.postedBy",
        select: "-password -stripeSession -role -courses",
      })
